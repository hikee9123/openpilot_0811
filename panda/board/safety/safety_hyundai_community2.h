...
const int HYUNDAI_COMMUNITY_PARAM_EV_GAS = 1;
const int HYUNDAI_COMMUNITY_PARAM_HYBRID_GAS = 2;

//bool hyundai_community = false;
bool hyundai_community_ev_gas_signal = false;
bool hyundai_community_hybrid_gas_signal = false;
...
static int hyundai_community_rx_hook(CAN_FIFOMailBox_TypeDef *to_push) {
  ...
  if (valid && (GET_BUS(to_push) == 0)) {
    ...
    if (addr == 593) {
      ...
      // enter controls on rising edge of ACC, exit controls on ACC off, for cars without long control
      if (addr == 1056) {  // SCC11
        // 1 bits: 0
        int cruise_engaged = GET_BYTES_04(to_push) & 0x1; // MainMode_ACC signal
        if (cruise_engaged && !cruise_engaged_prev) {
          controls_allowed = 1;
          puts("  SCC w/o long control: controls_allowed = 1");
        }
        if (!cruise_engaged) {
          controls_allowed = 0;
          puts("  SCC w/o long control: controls_allowed = 0");
        }
    ...
    } else if ((addr == 881) && hyundai_community_hybrid_gas_signal) {
